...
class MongoChatDataSource(
    ...
    override suspend fun getMessage(roomId: String): List<Message> {
        return messages.find<Message>(Filters.eq(Message::roomId.name,roomId)).toList()
    }

    override suspend fun sendMessage(message: Message): Boolean {
        val room = rooms.find(Filters.eq(Message::roomId.name, message.roomId)).toList().firstOrNull()
        val username = users.find(Filters.eq(User::userId.name,message.senderId)).toList().first().username
        val userPP = users.find(Filters.eq(User::userId.name,message.senderId)).toList().first().userPP

        return if (room == null) {
            val newRoom = Room(roomId = message.roomId, username = username, userPP = userPP, messages =  mutableListOf(message))
            rooms.insertOne(newRoom).wasAcknowledged()
        } else {
            val updatedRoom = room.copy(messages = room.messages.toMutableList().apply { add(message) })
            rooms.replaceOne(Filters.eq(Message::roomId.name, message.roomId), updatedRoom).wasAcknowledged()
        }
    }
